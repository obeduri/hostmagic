...
const formatted = formatBlock('test-proj', ['app.test', 'api.test-proj.test']);
assert.ok(formatted.includes('# BEGIN hostmagic:test-proj'));
assert.ok(formatted.includes('127.0.0.1  app.test'));
assert.ok(formatted.includes('127.0.0.1  api.test-proj.test'));
...
const updatedHosts = updateHostsContent(initialHosts, 'test-proj', ['test-proj.test']);
assert.ok(updatedHosts.includes('127.0.0.1 localhost'));
assert.ok(updatedHosts.includes('# BEGIN hostmagic:test-proj'));
assert.ok(updatedHosts.includes('127.0.0.1  test-proj.test'));
...
const updatedAgain = updateHostsContent(updatedHosts, 'test-proj', ['new.test-proj.test']);
assert.ok(updatedAgain.includes('127.0.0.1  new.test-proj.test'));
assert.ok(!updatedAgain.includes('127.0.0.1  test-proj.test'));
...
// Test 3b: updateHostsContent preserves multiple distinct projects without overwriting
console.log('Test 3b: updateHostsContent preserves multiple projects');
const otherProjectHosts = updateHostsContent(updatedAgain, 'other-proj', ['other.test']);
assert.ok(otherProjectHosts.includes('hostmagic:test-proj'), 'Original project must be preserved');
assert.ok(otherProjectHosts.includes('hostmagic:other-proj'), 'New project must be appended');
